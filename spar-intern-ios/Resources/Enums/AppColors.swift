//
//  AppColors.swift
//  spar-intern-ios
//
//  Created by Nikita Usov on 08.08.2024.
//

import SwiftUI

enum AppColors {
	static let surfaces001 = Color.surfaces001
	static let surfaces002 = Color.surfaces002
	
	static let primary001 = Color.primary001

	static let text002 = Color.text002
	static let text004 = Color.text004

	static let icons001 = Color.icons001
	static let icons002 = Color.icons002
	static let icons003 = Color.icons003

	static let additional004 = Color.additional004

	static let white = Color.white
	static let softGray = Color.softGray
	static let goodCardShadow = Color.goodCardShadow

	enum GoodBadge {
		static let priceHit = Color.pricesHit
		static let new = Color.new
		static let cardPrice = Color.cardPrice
	}
}
